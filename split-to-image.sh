...
# If the working dir already exists, exit early with an error
...
if [ -s $PAGES_FILE_PATH ]; then
    ...
    echo "${YELLOW}WARNING: You must define the page ranges of each individual letter in $PAGES_FILE_PATH before OCR processing.${NORMAL}"
fi
